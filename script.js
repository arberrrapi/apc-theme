...
// 3D Cube vertical flip functionality
document.addEventListener('DOMContentLoaded', function () {
  const faces = document.querySelectorAll('.cube-face');
  const iconGroups = document.querySelectorAll('.hero-icons');
  ...
  const initialIcons = document.querySelector('.cloud-icons');
  if (initialIcons) {
  ...
  function showIconsForFace(index) {
    ...
    const targetIcons = document.querySelector('.' + iconClassNames[index]);
    if (targetIcons) {
      setTimeout(() => {
